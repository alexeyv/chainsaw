...
sets CLI and model per role. The supervisor reads it once, at the run's first
command, and keeps it for the whole run; edit it before starting, not during. `cli` is `claude`, `cursor`, or `codex`. `model` is
