...
pub fn execute(store: &Store, command: Command) -> Result<()> {
    match command {
        ...
        Command::Prompt {
            ...
            if prepopulate {
                store.db.execute(
                    "update sessions set prepopulated_at=? where name=?",
                    params![now(), name],
                )?;
            }
            Ok(())
...
fn session_log(store: &Store, name: &str) -> Result<Option<PathBuf>> {
    let session_id = if let Some(session_id) = herdr_session_id(name) {
        store.db.execute(
    ...
    Ok(session_id.map(|session_id| store.logs_dir.join(format!("{session_id}.jsonl"))))
}
...
fn git_stdout(store: &Store, args: &[&str]) -> Result<String> {
    Ok(String::from_utf8_lossy(&git(store, args)?.stdout)
        .trim()
        .to_owned())
}

fn last_task_on(store: &Store, name: &str) -> Result<Option<Task>> {
    Ok(store
        .tasks()?
        .into_iter()
        .rev()
        .find(|task| task.implementer.as_deref() == Some(name) && task.state != "drafted"))
}

fn last_seen_commit(store: &Store, name: &str) -> Result<Option<String>> {
    Ok(last_task_on(store, name)?.and_then(|task| task.commit_sha.or(task.base_head)))
}
...
fn authored_files(store: &Store, name: &str) -> Result<Vec<String>> {
    let mut files = Vec::new();
    let mut seen = HashSet::new();
    for task in store
        .tasks()?
        .into_iter()
        .filter(|task| task.implementer.as_deref() == Some(name) && task.commit_sha.is_some())
    {
...
fn reuse_verdict(store: &Store, session: &Session) -> Result<Option<String>> {
    ...
    if store.tasks()?.iter().any(|task| {
        task.implementer.as_deref() == Some(&session.name)
            && matches!(task.state.as_str(), "dispatched" | "in_flight")
    }) {
        return Ok(Some("session is in flight".to_owned()));
    }
    let last = last_task_on(store, &session.name)?;
...
fn idle_pool(store: &Store) -> Result<Vec<IdleSession>> {
        ...
        .filter(|session| session.role == "implementer")
    {
        ...
        let since = last_seen_commit(store, &session.name)?;
        pool.push(IdleSession {
            stale: staleness(store, since.as_deref())?,
            files: authored_files(store, &session.name)?,
            session,
        });
    }
    Ok(pool)
...
fn describe_idle(store: &Store, idle: &IdleSession) -> Result<String> {
    ...
    if last_task_on(store, &session.name)?.is_none() {
        return Ok(format!(
...
fn cmd_launch(store: &Store, name: &str, options: LaunchOptions<'_>) -> Result<()> {
    if options.role == "implementer" {
        ...
        if !pool.is_empty() && !options.fresh {
            eprintln!(
                "supervisor: launch {name} refused — an idle implementer can take the next task:"
            );
            for idle in &pool {
    ...
    let session_id = json_string(&started, "/result/agent/agent_session/value")?;
    store.db.execute(
        "insert or replace into sessions(name,role,pane_id,tab_id,session_id,started_at,last_growth) values(?,?,?,?,?,?,?)",
        params![name, options.role, pane_id, tab_id, session_id, now(), now()],
    )?;
    store.event("launch", name)?;
    println!(
        "{}",
        json!({"name": name, "pane_id": pane_id, "tab_id": tab_id, "session_id": session_id})
    );
    Ok(())
}
...
fn cmd_prompt(store: &Store, name: &str, text: &str, wait: bool, timeout: u64) -> Result<()> {
    ...
    for attempt in 1..=PROMPT_ATTEMPTS {
        let path_before = session_log(store, name)?;
        ...
        while now() < deadline {
            let path = session_log(store, name)?;
            if path != path_before {
                offset = 0;
            }
...
fn cmd_task_new(
    ...
    {
        bail!("supervisor: --retry-of {retry_of} is not a failed task");
    }
    ...
    store.db.execute(
        "insert into tasks(text,predicted_files,predicted_lines,state,created_at,retry_of,predicted_file_list) values(?,?,?,?,?,?,?)",
        params![text, predicted_files, predicted_lines, "drafted", now(), retry_of, predicted_file_list],
    )?;
...
fn predecessor_unverified(store: &Store, task_id: i64) -> Result<Option<String>> {
    ...
    let mut statement = store
        .db
        .prepare("select state from task_log where task_id=?")?;
...
fn reuse_preamble(store: &Store, task: &Task, implementer: &str) -> Result<String> {
    let Some(since) = last_seen_commit(store, implementer)? else {
...
fn cmd_dispatch(store: &Store, task_id: i64, implementer: &str, reuse: bool) -> Result<()> {
        ...
        .find(|task| matches!(task.state.as_str(), "dispatched" | "in_flight"))
    {
        bail!(
            ...
            flying.implementer.unwrap_or_else(|| "-".to_owned()),
            flying.id
        );
    }
    ...
    let Some(session) = store.session(implementer)? else {
        bail!("supervisor: no session {implementer}; launch it first");
    };
    let prior = last_task_on(store, implementer)?;
    ...
    let preamble = if reuse {
        reuse_preamble(store, &task, implementer)?
    ...
    store.db.execute(
        "update tasks set implementer=?, reuse=? where id=?",
        params![implementer, i64::from(reuse), task_id],
    )?;
    ...
    store.db.execute(
        "update sessions set prepopulated_at=NULL where name=?",
        [implementer],
    )?;
    Ok(())
}
...
fn cmd_verify(store: &Store, task_id: i64) -> Result<()> {
    ...
    let log = match task.implementer.as_deref() {
        Some(implementer) => session_log(store, implementer)?,
        None => None,
    };
    let shas = log
        .as_deref()
...
fn harmless_after_gate(part: &str, run_dir: &Path) -> bool {
    ...
    match program {
        ...
        "echo" | "printf" | "true" | ":" | "ls" | "pwd" | "cat" | "head" | "tail" | "wc"
        | "test" | "[" | "date" | "cd" => true,
        "rm" => safe_rm(&tokens),
        _ => false,
    }
...
fn failures_in_lineage(store: &Store, task_id: i64) -> Result<i64> {
    ...
    while let Some(task) = current {
        ...
        current = match task.retry_of {
            Some(retry_of) => store.task(retry_of)?,
            None => None,
        };
    }
    Ok(failures)
...
fn cmd_calibrate(store: &Store, task_id: i64) -> Result<()> {
    ...
    let dispatched_at: Option<f64> = store
        .db
        .query_row(
            "select at from task_log where task_id=? and state='dispatched'",
            [task_id],
            |row| row.get(0),
        )
        .optional()?;
    let committed_at: Option<f64> = store
        .db
        .query_row(
            "select at from task_log where task_id=? and state='committed'",
            [task_id],
            |row| row.get(0),
        )
        .optional()?;
    ...
    let next_offset = store
        ...
        .find(|candidate| {
            candidate.id > task_id
                && candidate.implementer == task.implementer
    ...
    if end == 0
        && let Some(implementer) = task.implementer.as_deref()
    {
        end = store
            .session(implementer)?
            .map_or(0, |session| session.context_max);
    }
    let base = task.context_base.unwrap_or_default();
    ...
    let reuse = if task.reuse { ", reuse" } else { "" };
    println!(
...
fn cmd_state(store: &Store) -> Result<()> {
    ...
    for task in store.tasks()? {
        let mut statement = store
            .db
            .prepare("select state,at from task_log where task_id=?")?;
        let log = statement
        ...
        let reuse = if task.reuse {
            format!(
                "  reuse (context base {})",
                task.context_base.unwrap_or_default()
            )
        } else {
            String::new()
        };
        ...
        println!(
            ...
            task.id,
            task.state,
            task.implementer.as_deref().unwrap_or("-"),
    ...
    for session in store.sessions()? {
        ...
        if session.role == "implementer"
            && last_task_on(store, &session.name)?.is_some()
...
fn print_time_summary(store: &Store) -> Result<()> {
    let first: Option<f64> = store
        .db
        .query_row("select min(at) from task_log", [], |row| row.get(0))?;
    let mut busy = 0.0;
    for task in store.tasks()? {
        ...
        let end: Option<f64> = store
            .db
            .query_row(
                "select at from task_log where task_id=? and state in ('committed','verified','ingested','failed') order by at limit 1",
                [task.id],
                |row| row.get(0),
            )
            .optional()?;
...
fn cmd_context(store: &Store, name: Option<&str>) -> Result<()> {
    ...
    {
        let log = session_log(store, &session.name)?;
...
fn daemon(store: &Store, lead: &str) -> Result<()> {
    ...
    store.db.execute(
        "insert or ignore into sessions(name,role,started_at,last_growth) values(?,?,?,?)",
        params![lead, "lead", now(), now()],
    )?;
    ...
    while store.cfg("stopped")?.as_deref() != Some("1") {
            ...
            .filter(|session| session.stopped_at.is_none())
        {
            ...
            let log = session_log(store, name)?;
            ...
            store.db.execute(
                "update sessions set context=?, context_max=max(context_max,?), last_growth=case when ? then ? else last_growth end, kicked_at=case when ? then null else kicked_at end where name=?",
                params![context, context, grew, timestamp, grew, name],
            )?;
...
fn observe_implementer(
            ...
            .is_some_and(|status| matches!(status, "idle" | "done"))
    {
        ...
        store.db.execute(
            "update sessions set kicked_at=? where name=?",
            params![now(), session.name],
        )?;
