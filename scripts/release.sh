...
# Rust sources only: agent instructions such as AGENTS.md stay in the repo.
rsync -a --exclude tests.rs --exclude 'test_*.rs' \
  --include '*/' --include '*.rs' --exclude '*' "$ROOT/src/" "$OUT/src/"
