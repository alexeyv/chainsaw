...
class ReportingAndDaemonContractTests(SupervisorContractCase):
    ...
    def test_disposition_creates_the_documented_generated_view(self):
        task = self.new_task()
        self.assert_success(self.cli(
            "disposition", str(task), "The gate warning is actionable",
            "--verdict", "dropped",
        ...
        self.assertIn("task 1 · dropped · The gate warning is actionable", view)
        self.assertIn("not a product defect", view)

    def test_disposition_requires_an_existing_source_task(self):
        result = self.cli(
            "disposition", "99", "A finding", "--verdict", "dropped",
    ...
    def test_disposition_requires_an_existing_fix_task(self):
        ...
        result = self.cli(
            "disposition", str(source), "A finding", "--verdict", "task",
            "--fix-task", "99", "--reason", "worth fixing",
        )
